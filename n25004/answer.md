...
```md
踏み台サーバIP:[192.168.20.71]
WebサーバIP:  [192.168.20.70]
...
```md
| 送信先|ターゲット|ステータス|伝播済み|ルートオリジン |
| 0.0.0.0/0|igw-0e4adac12dbec9ed2|アクティブ|いいえ|ルートの作成|
| 192.168.20.0/24|local|アクティブ|いいえ|ルートテーブルの作成 |
```
...
```md
| 送信先|ターゲット|ステータス|伝播済み|ルートオリジン |
| 0.0.0.0/0|local|アクティブ|いいえ|ルートの作成|
...
- インバウンド:
    ...
    | bustions  | HTTP  | 80 | 0.0.0.0/0
アウトバウンド:
            |   type   |送信先| protcol |
            |anywere    | 送信先    | 0.0.0.0/0
...
- インバウンド
    | type  | port  |   source  |
    | ssh   |port   |   22  |   192.168.20.71/26
    | MYSQL/Aurora   |port   |   22  |   192.168.20.70
    | icmp   |port   |   22  |   0.0.0.0/0
- アウトバウンド
            |   type   |送信先| protcol |
            |anywere    | 送信先    | 0.0.0.0/0
...
```md
AWSは操作が簡単な反面,覚えることが沢山あるように感じました。
インスタンスやVPC,interfaceなどの基本構成はネットワークの基礎に沿って書かれてて分かりやすいのですが、クリック位置などの記憶が面倒に感じました。
しかし、それ以上に分かりにくいのが何処で詰まってるのか分かりづらいことです。クリック操作で簡単に出来るのが逆に学習しづらいと感じました
```
