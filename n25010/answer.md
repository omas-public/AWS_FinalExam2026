...
```md
踏み台サーバIP:[192.168.20.64/24]
WebサーバIP:  [192.168.20.64/24]
DBサーバIP:   [192.168.20.128/24]
```
...
```md
送信先　    192.168.20.0/24     0.0.0.0/0
ターゲット　local               internet gateway
...
```md
AWSの環境構築を通して、VPCやサブネット、EC2、RDSなどのAWSサービスを実際に設定し、クラウド上でシステムを構築する方法を学んだ。
セキュリティグループによるアクセス制御や、ネットワークの構築について学んだ。
実際に環境を構築することで、各サービスの役割やサービス間の連携について学んだ。
