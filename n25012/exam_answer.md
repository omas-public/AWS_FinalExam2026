...
```md
踏み台サーバIP:[192.168.20.64/26]
WebサーバIP:  [192.168.20.64/26]
DBサーバIP:   [192.168.20.128/26]
```
...
```md
| 送信元ネットワーク範囲 | ポート番号 | プロトコル | 用途 |
|---|---:|---|---|
| 自分のグローバルIP/32 | 22 | TCP | SSH接続 |
```
...
| 踏み台サーバのセキュリティグループ | 22 | TCP | SSH接続 |
```
...
| 送信元ネットワーク範囲 | ポート番号 | プロトコル | 用途 |
|---|---:|---|---|
| Webサーバのセキュリティグループ | 3306 | TCP | MySQL |
```
...
```md
今回の講義を通して、AWSを実際に使用しながら、基本的なWebサーバーの構築方法やルーティングなど、ネットワークに関する知識をより深く学ぶことができました。実際にサーバーを構築することで、これまで学んできたネットワークの知識がどのように使われているのかを理解できた点が良かったです。一方で、DBサーバーの作成や設定などは難しく感じる部分もありました。今回学んだことを復習し、今後はよりスムーズにサーバーやネットワークを構築できるようになりたいです。
