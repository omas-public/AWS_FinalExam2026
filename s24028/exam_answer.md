...
```md
VPCを利用したサブネットの分割などによって、公開・非公開を分け多層防御アーキテクチャの重要性がわかった。
クラウド環境におけるネットワークの構築手順の基礎、セキュリティ設計について身につけることができた。
```



申し訳ございません。ネットワーク周りの設定ミスに気づかず、Wordpressの導入まで進めることができませんでした。
ルートテーブルなどの設問に関しては、わかる範囲で記入させていただきました。
